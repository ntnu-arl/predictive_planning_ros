...
// #include "predictive_planner_common/graph_base.h"
...
BaseGraph::VertexDescriptor BaseGraph::addSourceVertex(int id) {
  source_ = boost::add_vertex(id, graph_);
  vertex_descriptors_[id] = source_;
  return source_;
}

BaseGraph::VertexDescriptor BaseGraph::addVertex(int id) {
  VertexDescriptor v = boost::add_vertex(id, graph_);
  vertex_descriptors_[id] = v;
  return v;
}

void BaseGraph::removeVertex(int id) {
  boost::remove_vertex(vertex_descriptors_[id], graph_);
}

BaseGraph::EdgeDescriptorPair BaseGraph::addEdge(int u_id, int v_id, double weight) {
...
void BaseGraph::removeEdge(int u_id, int v_id) {
  if (vertex_descriptors_.find(u_id) == vertex_descriptors_.end()) {
    return;
  }
  if (vertex_descriptors_.find(v_id) == vertex_descriptors_.end()) {
...
bool BaseGraph::findDijkstraShortestPaths(int src_id, ShortestPathsReport& rep) {
...
bool BaseGraph::findDijkstraShortestPaths(
...
int BaseGraph::getVertexID(VertexDescriptor v) {
  return boost::get(boost::get(boost::vertex_index, graph_), v);
}

int BaseGraph::getNumVertices() { return boost::num_vertices(graph_); }

int BaseGraph::getNumEdges() { return boost::num_edges(graph_); }

void BaseGraph::printResults(const ShortestPathsReport& rep) {
...
void BaseGraph::getEdgeIterator(
    std::pair<GraphType::edge_iterator, GraphType::edge_iterator>& ei) {
  ei = boost::edges(graph_);
}

std::tuple<int, int, double> BaseGraph::getEdgeProperty(
...
void BaseGraph::getVertexIterator(
    std::pair<GraphType::vertex_iterator, GraphType::vertex_iterator>& vi) {
  vi = boost::vertices(graph_);
}

int BaseGraph::getVertexProperty(GraphType::vertex_iterator v) {
  return boost::get(boost::get(boost::vertex_index, graph_), *v);
}